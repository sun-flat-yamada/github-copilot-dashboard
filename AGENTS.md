...
7. **Antigravity Two-Phase Governance & Deterministic Proceed Button**: In Google Antigravity, preserve canonical English headings and `ArtifactMetadata(RequestFeedback=true)` in `<appDataDir>\brain\<conversation-id>/implementation_plan.md` to deterministically trigger the UI `[Proceed]` button. Conclude turn immediately without chat re-summarization. See `.agents/skills/antigravity-two-phase-governance/SKILL.md`.
