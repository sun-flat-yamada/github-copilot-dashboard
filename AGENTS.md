...
7. **Antigravity Two-Phase Governance & Deterministic Proceed Button**: In Google Antigravity, preserve canonical English headings and `ArtifactMetadata(RequestFeedback=true)` in `<appDataDir>\brain\<conversation-id>/implementation_plan.md` to deterministically trigger the artifact-card `[Proceed]` button. Provide clickable `file:///` links, make zero non-artifact file edits in the planning turn, and conclude turn immediately. See `.agents/skills/antigravity-two-phase-governance/SKILL.md`.
