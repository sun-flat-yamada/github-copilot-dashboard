...
### 2.1 グローバルナビゲーション (ViewNavigation) & アクティブデータソース完全連動
- ナビゲーションバーの `ViewNavigation` に独立した専用ビューとして **「ディープ分析 (高度診断)」** (`deep_analysis`) を配置。
- アイコン: `BrainCircuit`。ワンクリックで全社横断のディープアナリティクスハブへ遷移。
- **アクティブ選択データへの完全連動**:
  - **Live Metrics**: 選択されたスコープ（月次・日次・カスタム）の確定テレメトリ（受諾数・提案数・日次履歴）に基づき診断。
  - **Monthly Usage Report (月次レポート)**: 選択月（`selectedReportMonth`）の月次ディープ分析アーカイブ（`data/processed/deep-analysis/{YYYY-MM}.json`）が存在する場合は優先ロード。未蓄積の月次レポートに対しても、日別トレンド（`daily_trends`）と明細からプロファイルを動的合成（`adaptReportToProfiles`）し、按分推定診断を実施。
  - **User Upload (ローカルファイルドロップ)**: ユーザーが手動アップロードしたCSV/JSONデータに対しても即座に動的適応し、非効率パターン診断を実行可能。
  - **タグANDフィルター連動**: グローバルで選択されたタグ条件が診断対象ユーザー母数へ完全に連動反映。
  - ビュー上部には「分析データソース情報バッジ」（確定テレメトリ / 按分推定モード、対象ユーザー数）を常時明示。

### 2.2 コンテキスト起動 (Contextual Deep-Link)
- **ユーザー明細 & ランキングテーブル (`UserDetailTable`)**: 各行のアクション列に「診断」ボタンを設置。クリックすると対象ユーザーが自動選択された状態で専用ビューが起動。
- **月次レポートユーザー明細テーブル (`MonthlyReportUserTable`)**: アクション列に「診断」ボタンを設置。月次レポート閲覧中からもワンクリックで対象ユーザーのディープ分析へ遷移可能。
...
- **対象**: 選択された1ユーザーの利用実績（`UserUsageProfile.daily_history`）。
  - **ユーザー選択 & 左右送り**: セレクトボックスによる直接選択に加え、前後のユーザーへダイレクトに遷移可能な左右送りボタン（`ChevronLeft` / `ChevronRight`）および位置インデックス表示（例: `3 / 40`）を常設。リストの先頭・末尾での無効化制御とツールチップ案内をサポート。
- **対象区間の切り替え**:
  - `[前1カ月間]` (デフォルト: 直近30日間)
  - `[当日]` (直近1日)
  - `[1週間前まで]` (直近7日間)
  - `[期間指定 (カスタム)]`: クリック時に自動で `custom` スコープへ切り替わり、カレンダー入力パネルが展開。任意の開始日（Start Date）および終了日（End Date）を `<input type="date">` で入力し「この期間で診断を更新」をクリックすることで、該当日付範囲のデータを動的フィルタリング。選択中の再クリックによるパネル開閉トグルにも対応。
