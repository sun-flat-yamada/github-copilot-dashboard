...
- **タグ/スコープ絞り込みとの連動 (`利用割合 (%)` 再集計)**:
  - Tag フィルターやスコープ（対象月・組織等）を変更した場合、各モデルの利用実績・利用割合 (%) は必ずその時点の「アクティブ選択されている分析対象データ」のみを基に再集計される。
  - **Live Metrics**: ユーザーごとのモデル別リクエスト数 (`model_usage_totals`) を保持するため、タグ AND フィルター適用後のユーザー集合から厳密に再集計する。
...
### 2.6.1 モデル選択切り替え時のレーダーポリゴン滑らか変形（モーフィング）トランジション仕様
- **課題と目的**:
  - 従来はアクティブモデルを切り替えるたびに、新モデルのレーダー多角形が原点（ゼロ）から広がるように再描画されていたため、切り替え前後のモデル間で「どの評価軸がどちら方向にどれだけ動いたか」（例: SWEスコアが何点伸びたか、速度がどれだけ低下したか）を視覚的に瞬時に比較することが難しかった。
  - 切り替え前のモデルの枠線形状から新モデルの枠線形状へと各軸の頂点が連続的に補間移動（モーフィング変形）するトランジションを導入し、モデル間の強み・弱みの相対的差分を直感的に把握可能とする。
- **アクティブRadarのアニメーション仕様**:
  - アクティブ描画の `<Radar>` コンポーネントに固定キー（`key="active-focused-radar"`）を付与することで、モデル切り替え時のコンポーネントアンマウント／破棄を回避し、Recharts内部の `previousPointsRef`（前モデルの6頂点座標）から新モデルの頂点座標への滑らかなジオメトリ補間（`matchByIndex` による頂点アニメーション）を発火させる。
  - アニメーションパラメータ:
    - `isAnimationActive={true}`
    - `animationDuration={600}`（操作応答性と変化の視認性を両立する 600ms）
    - `animationEasing="ease-out"`（終盤に向かって減速し、確定値に着地する自然なイージング曲線）
  - カラー遷移:
    - SVGストロークおよび塗り色に対して CSS `transition: stroke 0.4s ease, fill 0.4s ease` を適用し、ジオメトリ変形と調和したブランドカラーのフェード切り替えを行う。
- **非表示（空状態）からの初期表示仕様**:
  - 選択モデルが0件の未選択状態（または初期非表示状態）からモデルが選択された場合は、変更前モデルが存在しないため、従来通り中心点（ゼロ）から各軸へと外側に広がるエントランスアニメーションを描画する。
- **背景比較モデル群（非アクティブ点線）の静止固定仕様**:
  - 背景に表示される非アクティブな選択モデル群（点線）は `isAnimationActive={false}` とし、アクティブ切り替えに伴って背景点線がゼロから再描画される視覚ノイズを防止。ユーザーの視線を前面のアクティブ実線のモーフィング変形に集中させる。
