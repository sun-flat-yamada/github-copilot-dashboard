...
- **3段階の表示モード切り替え (`sidebarMode`)**:
  ...
  - **省幅表示 (`compact`)**:
    - 省幅（約 210px〜220px）。AIモデル略称（モデル派閥とバージョン番号を省略せず保持。例: `gpt-5.6-luna`, `gpt-6-astra`, `opus-5`, `opus-4.8`, `sonnet-4.6`, `sonnet-3.7`, `gemini-3.8-flash` 等、`getModelShortName` による標準略称形）でコンパクトに表示。
    - マウスホバー時にフル名称、ベンダー、Tier、総合スコア、組織内利用シェア%をツールチップ表示。
  - **非表示 (`collapsed`)**:
    - 左側フレームを折りたたみ、画面幅を最大限ウィジェット表示に開放。画面左端に常駐するコンパクトな再展開ボタン（`[▶ モデル選択 (X)]`）から1クリックで復帰可能。
- **選択状態・アクション仕様**:
