  ...
  return (
    <div className="min-h-screen bg-[#090d13] text-slate-100 flex flex-col selection:bg-indigo-500 selection:text-white">
      ...
      <header className="border-b border-slate-800/80 bg-slate-950/80 backdrop-blur sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 min-h-16 py-2.5 sm:py-0 flex items-center justify-between gap-2 sm:gap-4 flex-wrap sm:flex-nowrap">
          {/* ブランド & タイトル & Aboutボタン */}
          ...
          <div className="flex items-center space-x-2 sm:space-x-3 text-xs flex-shrink-0">
            {/* モード切替スイッチ (レスポンシブ: xl以上でフル表示、xl未満でドロップダウン) */}
            <ModeSwitcher
              currentMode={appMode}
            ...
            {/* 生成元 GitHub リポジトリリンク (Forkセーフ・動的解決) */}
            <a
              href={repoInfo.url}
              target="_blank"
              rel="noopener noreferrer"
              className="flex items-center space-x-1.5 text-slate-400 hover:text-slate-100 bg-slate-900 border border-slate-800 hover:border-slate-700 px-2.5 sm:px-3 py-1.5 rounded-xl transition-all shadow-sm group cursor-pointer"
              title={`GitHubリポジトリを開く: ${repoInfo.owner}/${repoInfo.name}`}
            >
              <GitFork className="w-3.5 h-3.5 text-slate-400 group-hover:text-indigo-400 flex-shrink-0 transition-colors" />
              <span className="hidden md:inline font-mono">
                {repoInfo.owner}/{repoInfo.name}
              </span>
              <span className="md:hidden font-mono truncate max-w-[80px]">
                {repoInfo.name}
              </span>
